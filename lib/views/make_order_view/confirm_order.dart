...
import 'package:crispychikis/views/make_order_view/online_payment.dart';
...
class ConfirmOrderState extends State<ConfirmOrder> {
  ...
  List<String> paymentMethods = ['Efectivo', 'En linea'];
                      ...
                      dropDownValue == 'Efectivo'
                          ...
                          builder: (context, state) =>Center(
                              child: CustomButton(
                                  text: 'Pagar en linea',
                                  onPressed: () async{
                                    final int intAmount=100*state.total.toInt();
                                    final user=state.username;
                                    final bool? isPay=await Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                            builder: (context) => PayWebView(
                                                amountInCents: intAmount,
                                                username: user))) ?? false;
                                    setState(()=>isPaid=isPay!);
                                  })))
                    ],
                  )),
              BlocBuilder<MakeOrderBloc, MakeOrderState>(
                              ...
                              FloatingActionButton.extended(
                                  ...
                                  label: Text('Regresar',
                                      style: GoogleFonts.nunito(
                                          color: colorsPalete['pink'],
                                          fontSize: 22,
                                          fontWeight: FontWeight.w700))):SizedBox(height: 55),
                              SizedBox(height: 6),
                              dropDownValue == 'Efectivo' || isPaid ?
                              CustomButton(
                                  text: 'HACER PEDIDO',
                                  onPressed: () {
